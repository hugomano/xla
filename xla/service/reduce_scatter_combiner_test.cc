...
TEST_F(ReduceScatterCombinerTest, PreservesMetadata) {
  absl::string_view hlo_string = R"(
    ...
    ENTRY entry {
      ...
      %rs.0 = f32[16] reduce-scatter(%param.0), replica_groups={{0,1}}, dimensions={0}, to_apply=%add, metadata={op_type="test_type" op_name="test_name"}
      %rs.1 = f32[16] reduce-scatter(%param.1), replica_groups={{0,1}}, dimensions={0}, to_apply=%add, metadata={op_type="test_type" op_name="test_name"}
  ...
  TF_ASSERT_OK_AND_ASSIGN(auto module,
                          RunPass(hlo_string, /*expect_change=*/true));
  OpMetadata metadata;
  metadata.set_op_type("test_type");
  metadata.set_op_name("test_name");
  auto combined_all_gather = op::Metadata(metadata);
