...
TEST_F(AllGatherCombinerTest, PreservesMetadata) {
  absl::string_view hlo_string = R"(
    ...
    ENTRY entry {
      param0 = f32[32] parameter(0)
      param1 = f32[32] parameter(1)
      allgather0 = f32[128] all-gather(param0), replica_groups={}, dimensions={0}, metadata={op_type="test_type" op_name="test_name"}
      allgather1 = f32[128] all-gather(param1), replica_groups={}, dimensions={0}, metadata={op_type="test_type" op_name="test_name"}
      ROOT tuple = (f32[128], f32[128]) tuple(allgather0, allgather1)
    }
  )";
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> module,
